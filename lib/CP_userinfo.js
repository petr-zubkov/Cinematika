...
var cache = new LRU({ max: 100000 });
var blacklist = new LRU({ max: 100000 });
...
module.exports = function(lookup) {
  ...
  return function(req, res, next) {
    ...
    req.userinfo.bot = req.userinfo.bot || {};
    req.userinfo.ip =
      req.userinfo.ip ||
      (
        (
    ...
    if (config_ru_domain) {
      if (req.userinfo.bot.main) {
        ...
        if (
          ...
          info.country.names.en === 'Russia'
        ) {
          res.cookie('CP_russia', '1', {
            maxAge: 86400000,
            httpOnly: true
          });
    ...
    if (req.userinfo.bot.main) {
      ...
      var regexpId = new RegExp(prefixId + '([0-9]{1,10})', 'i');
      ...
      req.userinfo.domain =
        (modules.tv.data.subdomain
          ...
          ? config_botdomain
            ...
            : ''
          : config_subdomain) +
        (req.userinfo.bot.main && config_bomain
          ? config_bomain
          : config_domain) +
        req.userinfo.port +
