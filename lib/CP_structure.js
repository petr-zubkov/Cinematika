...
function structureMovie(movies, options) {
  ...
  return movies.map(function(movie) {
    ...
    movie = {
      ...
      directors_arr2:
        movie.director && config.default.people_image
          ? createPeopleArr('director', movie.director)
          : [],
      actors_arr2:
        movie.actor && config.default.people_image
          ? createPeopleArr('actor', movie.actor)
          : [],
    ...
    try {
      ...
      //   if (movie.custom['player' + n]) {
      //     movie.player +=
      //       (movie.player ? ',' : '') + movie.custom['player' + n];
      //   }
      // });
      var seasons = [];
      Object.keys(movie.custom).forEach(function(c) {
        if (
          /^s[0-9]+e[0-9]+$/i.test(c) &&
          typeof movie.custom[c] === 'object' &&
          [1, 2, 3, 4, 5, 6, 7, 8, 9]
            .map(function(n) {
              return movie.custom[c]['player' + n];
            })
      ...
      if (seasons.length) {
        var last_episode = seasons.sort().reverse()[0];
        movie.season = last_episode.split('s')[1].split('e')[0];
        movie.episode = last_episode.split('e')[1];
      }
    } catch (e) {
      movie.custom = {};
    }
  ...
  function createCategoryUrl(type, items) {
    ...
    itemsArr = itemsArr.map(function(item) {
      return (
        ...
        config.urls.slash +
        CP_translit.text(
          config.urls.translit_lower_case ? item.toLowerCase() : item,
          undefined,
          config.urls[type]
        ) +
        '"' +
        (config.default.people_image &&
        (type === 'actor' || type === 'director')
          ? ' class="Cinematika-tooltips"'
          : '') +
        '>' +
        item +
        (config.default.people_image &&
        (type === 'actor' || type === 'director')
          ? '<span style="position:absolute;visibility:hidden"><img src="' +
            (config.default.people_image_domain || '') +
            '/files/poster/people/' +
            item.replace('Ё', 'Е').replace('ё', 'е') +
            '.jpg" alt="' +
            item +
            '"></span>'
          : '') +
  ...
  function createPeopleArr(type, items) {
    var itemsArr = [];
...
function structureCategories(type, movies, options) {
  ...
  function createCategoryUrl(type, category) {
    return (
      ...
      config.urls.slash +
      CP_translit.text(
        config.urls.translit_lower_case ? category.toLowerCase() : category,
        undefined,
        config.urls[type]
      )
    );
  ...
  function bb_codes(text) {
    return text
      ...
      .replace(/\[spoiler](.*?)\[\/spoiler]/gi, function(match, p1) {
        return (
          '<span class=Cinematika-comment-spoiler data-comment-spoiler="' +
          p1.replace(/"/g, '&quot;') +
          '">👻&nbsp;' +
          config.l.spoiler +
          '</span>'
      ...
      .replace(/\[search](.*?)\[\/search]/gi, function(match, p1) {
        return (
          '<span class=Cinematika-comment-search data-comment-search="' +
          p1.replace(/"/g, '&quot;') +
          '">🔍&nbsp;' +
          p1 +
          '</span>'
      ...
      .replace(
        /\[b](.*?)\[\/b]/gi,
        '<span class=Cinematika-comment-b>$1</span>'
      )
      .replace(
        /\[i](.*?)\[\/i]/gi,
        '<span class=Cinematika-comment-i>$1</span>'
      )
      .replace(
        /\[url](.*?)\[\/url]/gi,
