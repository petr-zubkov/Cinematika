...
docker_install() {
    if [ "${CP_OS}" != "alpine" ] && [ "${CP_OS}" != "\"alpine\"" ]; then
        if [ "`basename "${0}"`" != "Cinematika" ] || [ "${1}" != "" ]; then
            ...
            wget -qO /usr/bin/Cinematika https://gitlab.com/Cinematika/Cinematika/raw/master/Cinematika.sh -o /dev/null && \
...
1_install() {
    ...
    if [ ! "$(docker network ls | grep Cinematika)" ]; then
        docker network create \
            --driver bridge \
            Cinematika >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
    fi

    # docker build -t Cinematika/docker https://github.com/Cinematika/Cinematika.git
    docker pull Cinematika/docker:latest >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
    ...
    docker run \
        ...
        ${EXTERNAL_DOCKER} \
        Cinematika/docker:latest >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
    ...
    if [ "`docker ps -aq -f status=running -f name=^/nginx\$ 2>/dev/null`" != "" ]; then
        ...
        && [ "`netstat -tunlp | grep :::80`" = "" ]; then
            # docker build -t Cinematika/nginx https://github.com/Cinematika/Cinematika.git#:config/default/nginx
            # docker build -t Cinematika/fail2ban https://github.com/Cinematika/Cinematika.git#:config/default/fail2ban
            # docker build -t Cinematika/filestash https://github.com/Cinematika/Cinematika.git#:config/default/filestash
            ...
            docker run \
                ...
                -p 80:80 \
                -p 443:443 \
                Cinematika/nginx:latest >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
            ...
            docker run \
                ...
                Cinematika/fail2ban:latest >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
            ...
            docker run \
                ...
                --restart always \
                --network Cinematika \
                Cinematika/filestash >>/var/log/docker_install_"$(date '+%d_%m_%Y')".log 2>&1
            ...
            while [ "${FILESTASH_RUN}" != "50" ]; do
                ...
                if [ "`docker ps -aq -f status=running -f name=^/filestash\$ 2>/dev/null`" != "" ]; then
                    docker exec "${LOCAL_DOMAIN_}" /usr/bin/Cinematika container ftp
                    FILESTASH_RUN=50
                fi
            done
...
2_update() {
    ...
    if [ "${LOCAL_MEGA_EMAIL}" != "" ] && [ "${LOCAL_MEGA_PASSWORD}" != "" ]; then
        ...
        docker exec "${LOCAL_DOMAIN_}" rclone config create Cinematika "${LOCAL_CONF_STORAGE[@]}" \
    ...
    CHECK_MEGA=$(docker exec -t "${LOCAL_DOMAIN_}" rclone config show 2>/dev/null | grep "Cinematika")
    ...
    if [ "${CHECK_MEGA}" = "" ]; then
        ...
        _content
        printf "     ~# Cinematika b %s config \"email\" \"pass\"" "${LOCAL_DOMAIN}"
        _br
        _content
        ...
        _content
        printf "     ~# Cinematika b %s ftp \"user\" \"pass\" \"host\"" "${LOCAL_DOMAIN}"
        _br
        _content
    ...
    if [ "${DISABLE_SSL}" = "" ]; then
        docker exec "${LOCAL_DOMAIN_}" /usr/bin/Cinematika container protocol "https://"
    fi
...
3_backup() {
    ...
    RCS=$(docker exec -t "${LOCAL_DOMAIN_}" rclone config show 2>/dev/null | grep "Cinematika")
    ...
    if [ "${LOCAL_ACTION}" = "config" ] || [ "${LOCAL_ACTION}" = "3" ] || [ "${RCS}" = "" ]; then
        if [ "${LOCAL_MEGA_EMAIL}" != "" ] && [ "${LOCAL_MEGA_PASSWORD}" != "" ]; then
            ...
            docker exec "${LOCAL_DOMAIN_}" rclone config create Cinematika "${LOCAL_CONF_STORAGE[@]}" \
                >>/var/log/docker_backup_"$(date '+%d_%m_%Y')".log 2>&1
            sleep 10
            CHECK_MKDIR=$(docker exec -t "${LOCAL_DOMAIN_}" rclone mkdir Cinematika:/check-connection 2>/dev/null)
            sleep 3
            CHECK_RMDIR=$(docker exec -t "${LOCAL_DOMAIN_}" rclone rmdir Cinematika:/check-connection 2>/dev/null)
            ...
            if [ "${LOCAL_ACTION2}" = "create" ] || [ "${LOCAL_ACTION2}" = "1" ]; then
                docker exec "${LOCAL_DOMAIN_}" /usr/bin/Cinematika container backup create hand \
                    >>/var/log/docker_backup_"$(date '+%d_%m_%Y')".log 2>&1
            elif [ "${LOCAL_ACTION2}" = "restore" ] || [ "${LOCAL_ACTION2}" = "2" ]; then
                docker exec "${LOCAL_DOMAIN_}" /usr/bin/Cinematika container backup restore "${LOCAL_DOMAIN2}" \
                    >>/var/log/docker_backup_"$(date '+%d_%m_%Y')".log 2>&1
            ...
            _content
            printf "     ~# Cinematika b %s create" "${LOCAL_DOMAIN}"
            _br
            printf "     ~# Cinematika b %s restore" "${LOCAL_DOMAIN}"
            _br
            _content
            _s
            exit 0
        ...
        if [ "${LOCAL_ACTION}" = "create" ] || [ "${LOCAL_ACTION}" = "1" ]; then
            ...
            docker exec "${LOCAL_DOMAIN_}" /usr/bin/Cinematika container backup restore \
                >>/var/log/docker_backup_"$(date '+%d_%m_%Y')".log 2>&1
            docker exec nginx nginx -s reload \
                >>/var/log/docker_backup_"$(date '+%d_%m_%Y')".log 2>&1
        fi
...
4_theme() {
    ...
    if [ -d "/home/${LOCAL_DOMAIN}/themes/${LOCAL_THEME}" ]; then
        ...
        then
            ...
            git clone https://${GIT_SERVER}/Cinematika/Theme-"${LOCAL_THEME}".git \
...
7_mirror() {
    ...
    docker start "${LOCAL_MIRROR_}" \
        >>/var/log/docker_mirror_"$(date '+%d_%m_%Y')".log 2>&1
    sleep 10
    docker exec "${LOCAL_MIRROR_}" /usr/bin/Cinematika container mirror \
        >>/var/log/docker_mirror_"$(date '+%d_%m_%Y')".log 2>&1
    sleep 10
    docker exec nginx nginx -s reload \
        >>/var/log/docker_mirror_"$(date '+%d_%m_%Y')".log 2>&1
...
8_remove() {
    ...
    # docker build -t Cinematika/docker https://github.com/Cinematika/Cinematika.git
    docker pull Cinematika/docker:latest >>/var/log/docker_remove_"$(date '+%d_%m_%Y')".log 2>&1
    ...
    if [ "${LOCAL_FULL}" != "" ]; then
        ...
        # docker build -t Cinematika/nginx https://github.com/Cinematika/Cinematika.git#:config/default/nginx
        docker pull Cinematika/nginx:latest >>/var/log/docker_remove_"$(date '+%d_%m_%Y')".log 2>&1
...
#        # docker build -t Cinematika/fail2ban https://github.com/Cinematika/Cinematika.git#:config/default/fail2ban
#        docker pull Cinematika/fail2ban:latest >>/var/log/docker_remove_"$(date '+%d_%m_%Y')".log 2>&1
...
#        # docker build -t Cinematika/filestash https://github.com/Cinematika/Cinematika.git#:config/default/filestash
#        docker pull Cinematika/filestash:latest >>/var/log/docker_remove_"$(date '+%d_%m_%Y')".log 2>&1
    fi
...
option() {
    ...
    printf "${C}---- ${G}u)${NC} update ${S}------------------- Upgrade Cinematika System ${C}----\n"
...
sh_yes() {
    if [ -f "/home/${CP_DOMAIN}/process.json" ]; then
        ...
        _content
        if [ -n "${1}" ]; then
            docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container "passwd" "${1}" \
...
docker_run() {
    if [ ! -d "/home/${CP_DOMAIN}/config/production" ]; then
        find /var/Cinematika -maxdepth 1 -type f -iname '\.gitkeep' -delete
        cp -rf /var/Cinematika/* /home/"${CP_DOMAIN}"
        rm -rf /var/Cinematika/* /var/${CP_THEME:?}
        ...
        git clone https://${GIT_SERVER}/Cinematika/Theme-${CP_THEME}.git /var/${CP_THEME}
        ...
        if [ ! -f "/var/log/${CP_DOMAIN_}.pid" ]; then
            nohup /usr/bin/Cinematika container cron run >"/var/log/${CP_DOMAIN_}.log" 2>&1 &
...
docker_cron() {
    nohup /usr/bin/Cinematika torrent "${CP_DOMAIN}" upload \
...
docker_restore() {
    WEB_DIR=${1:-${CP_DOMAIN}}
    LATEST_DIR=${2:-latest}
    RCS=$(rclone config show 2>/dev/null | grep "Cinematika")
    if [ "${RCS}" = "" ]; then exit 0; fi
    docker_stop
    rm -rf /var/mega/new && mkdir -p /var/mega/new
    mkdir -p /home/"${CP_DOMAIN}"/config/custom
    sleep 5
    sleep 3; rclone -vv --ignore-size copy Cinematika:"${WEB_DIR}"/"${LATEST_DIR}"/config.tar /var/mega/new/"${CP_DOMAIN}"/
    sleep 3; rclone -vv --ignore-size copy Cinematika:"${WEB_DIR}"/"${LATEST_DIR}"/themes.tar /var/mega/new/"${CP_DOMAIN}"/
    sleep 5
...
#    done
    ...
    tar --ignore-failed-read \
        ...
        themes/"${THEME_NAME}" \
        files
    sleep 3; rclone delete --rmdirs Cinematika:"${CP_DOMAIN}"/"${BACKUP_NOW}" &> /dev/null
    if [ "${BACKUP_DAY}" != "10" ]; then
        rclone delete --rmdirs Cinematika:"${CP_DOMAIN}"/"${BACKUP_DELETE}" &> /dev/null;
        rclone rmdirs Cinematika:"${CP_DOMAIN}"/"${BACKUP_DELETE}" &> /dev/null;
    fi
    sleep 3; rclone delete --rmdirs Cinematika:"${CP_DOMAIN}"/latest &> /dev/null
    sleep 3; rclone -vv --ignore-size copy /var/mega/"${CP_DOMAIN}"/config.tar Cinematika:"${CP_DOMAIN}"/"${BACKUP_NOW}"/
    sleep 3; rclone -vv --ignore-size copy /var/mega/"${CP_DOMAIN}"/themes.tar Cinematika:"${CP_DOMAIN}"/"${BACKUP_NOW}"/
    sleep 3; rclone -vv --ignore-size copy /var/mega/"${CP_DOMAIN}"/config.tar Cinematika:"${CP_DOMAIN}"/latest/
    sleep 3; rclone -vv --ignore-size copy /var/mega/"${CP_DOMAIN}"/themes.tar Cinematika:"${CP_DOMAIN}"/latest/
    KILOBYTE_ALL=$(df -k /home | tail -1 | awk '{print $4}')
    ...
    CHECK_MKDIR=$(rclone mkdir Cinematika:/check-connection 2>/dev/null)
    sleep 3
    CHECK_RMDIR=$(rclone rmdir Cinematika:/check-connection 2>/dev/null)
    if [ "${CHECK_MKDIR}" != "" ] || [ "${CHECK_RMDIR}" != "" ]; then
        _header "ERROR"
        _content
        _content "Cannot connect to backup storage."
...
success_install(){
    ...
    _content "root@vps:~# Cinematika backup"
    _content
    _content "Issues: github.com/Cinematika"
    _content
    _s
}
    ...
    case ${OPTION} in
        ...
        "zero" )
            ...
            if [ "${YES}" != "ДА" ] && [ "${YES}" != "Да" ] && [ "${YES}" != "да" ] && [ "${YES}" != "YES" ] && [ "${YES}" != "Yes" ] && [ "${YES}" != "yes" ] && [ "${YES}" != "Y" ] && [ "${YES}" != "y" ]; then
                docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container zero "NOT" \
                    >>/var/log/docker_zero_"$(date '+%d_%m_%Y')".log 2>&1
            else
                docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container zero \
                    >>/var/log/docker_zero_"$(date '+%d_%m_%Y')".log 2>&1
            fi
            exit 0
        ;;
        "zero_rt"|"zero_realtime" )
            ...
            if [ "${YES}" != "ДА" ] && [ "${YES}" != "Да" ] && [ "${YES}" != "да" ] && [ "${YES}" != "YES" ] && [ "${YES}" != "Yes" ] && [ "${YES}" != "yes" ] && [ "${YES}" != "Y" ] && [ "${YES}" != "y" ]; then
                ...
                docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container zero_rt \
        ...
        "hand" )
            ...
            sh_not
            _s "${2}"
            docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container hand "${3}"
            exit 0
        ;;
        "main_mirror" )
            _br "${2}"
            ...
            sh_not
            _s "${2}"
            docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container mirror
            exit 0
        ;;
        "movies" )
            _br "${2}"
            ...
            sh_not
            _s "${2}"
            docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container movies
            exit 0
        ;;
        "reload"|"actual"|"available"|"reindex"|"speed"|"cron" )
            ...
            sh_not
            _s "${2}"
            docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container "${1}" "${3}"
            _br
            exit 0
        ;;
        "container" )
        ...
        "mode" )
            ...
            sh_progress
            docker exec -t "${CP_DOMAIN_}" /usr/bin/Cinematika container reload &>/dev/null
            sh_progress 100
            exit 0
        ;;
        "l"|"ll"|"lp"|"lm"|"lms"|"ls"|"llp"|"log"|"logs"|"live"|"lb"|"lbt"|"lbf"|"lbb" )
            ...
            _content
            if docker network ls | grep -q Cinematika
            then
            ...
            _content
            docker exec -t "${CP_DOMAIN_}" /usr/bin/Cinematika container logs
            exit 0
        ;;
    ...
    location / {return 444;}
        ...
        "import" )
            if [ "${2}" = "dle" ]; then
                ...
                CREATE_EXPORT=$(wget -qO- "http://${IMPORT_DOMAIN}/dle2Cinematika.php?domain=${CP_DOMAIN}")
                if [ "${CREATE_EXPORT}" != "ok" ]; then
                    _header "ERROR"
                    _content
                    _content "The website ${IMPORT_DOMAIN} is temporarily unavailable,"
                    _content "please try again later."
                    _content "http://${IMPORT_DOMAIN}/dle2Cinematika.php?domain=${CP_DOMAIN}"
                    _content
                    _s
                    exit 0
                else
        ...
        "app" )
            ...
            docker run \
                ...
                Cinematika/app:latest \
                --name "${CP_DOMAIN_}" \
                --platform "${NAME_OS}" \
                --arch "x64" \
                --app-copyright "Cinematika App" \
                --app-version "${CP_VER}" \
                --icon "/icons/icon.png" \
                --width "1280px" \
                --height "800px" \
                --min-width "0" \
                --min-height "0" \
                --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:99.0) Gecko/20100101 Firefox/99.0 Cinematika App" \
                --ignore-certificate \
                --insecure \
                --internal-urls ".*" \
        ...
        "ms"|"mailserver"|"mail"|"setup.sh"|"./setup.sh" )
            if [ "${2}" = "help" ]; then
                ...
                sed -Ei "s/\\\$0/Cinematika ms/" /usr/bin/mailcinema
        ...
        "temp"|"template"|"design"|"cinematheme"|"ct"|"uncss" )
            ...
            if [ "${3}" = "-h" ]; then
                ...
                docker exec -it "${CP_DOMAIN_}" /usr/bin/Cinematika container cinematheme "${@}"
            else
                _line
                ...
                if [ "${1}" = "uncss" ]; then
                  ...
                  docker exec -it "${CP_DOMAIN_}" /usr/bin/Cinematika container cinematheme "${ARRAY_PAGE[@]}"
                fi
                _br
            fi
            exit 0
        ...
        "cms" )
            ...
            if [ "${NAME_CMS}" = "backup" ] && [ "${4}" = "create" ]; then
                if [ -f "/var/lib/Cinematika/dump/backup.sql" ]; then
                    echo "ERROR: Backup file found /var/lib/Cinematika/dump/backup.sql"
                    exit 0
                fi
                docker exec mysql sh -c 'exec mysqldump -A -uroot' \
                    > "/var/lib/Cinematika/dump/backup.sql"
                echo "SUCCESS: Backup file /var/lib/Cinematika/dump/backup.sql"
                exit 0
            fi
            if [ "${NAME_CMS}" = "backup" ] && [ "${4}" = "restore" ]; then
                if [ ! -f "/var/lib/Cinematika/dump/restore.sql" ]; then
                    echo "ERROR: Restore file not found /var/lib/Cinematika/dump/restore.sql"
                    exit 0
                fi
                docker exec -i mysql sh -c 'exec mysql -uroot' \
                    < "/var/lib/Cinematika/dump/restore.sql"
                echo "SUCCESS: Restore file /var/lib/Cinematika/dump/restore.sql"
                exit 0
            fi
            mkdir -p /var/lib/Cinematika/php
            mkdir -p /var/lib/Cinematika/mysql
            mkdir -p /var/lib/Cinematika/dump
            ...
            if [ ! "$(docker ps -a | grep mysql)" ]; then
                docker run \
                    ...
                    --restart always \
                    --network Cinematika \
                    -v /var/lib/Cinematika/mysql:/var/lib/mysql \
            ...
            if [ "${NAME_CMS}" = "wordpress" ]; then
            ...
            elif [ "${MYSQL_USER}" != "" ] && [ "${MYSQL_PASSWORD}" != "" ] && [ "${MYSQL_DATABASE}" != "" ]; then
                {
                    echo "<html>"
                    echo "<head>"
                    echo "    <title>Hello Cinematika</title>"
                    echo "</head>"
                    echo "<body>"
                    echo "    <?php"
        ...
        "redirect" )
            if [ "${2}" = "" ] || [ "${3}" = "" ]; then
                echo "ERROR: Cinematika redirect example.co hd.example.com"
                exit 0
            fi
...
---------------------------------------------------
")
                        CRASH+=("$(docker exec -t "${DD_}" Cinematika ping 2>/dev/null)")
                        CRASH+=("
---------------------------------------------------
")
                        CRASH+=("PS ${DD}")
...
")
        ...
        "uptimerobot" )
            ...
            if [ -n "${2}" ] && [ "${2}" != "push" ]; then
                ...
                PONG1=$(docker exec -t "${DD_}" /usr/bin/Cinematika ping 2>/dev/null)
                if [ "${PONG1}" != "pong" ]; then
                ...
                for D in /home/*; do
                    ...
                    if { [ -f "${D}/app.js" ] || [ -f "${D}/index.php" ]; } && [ "${PONG1}" = "pong" ]; then
                        if [ "$(grep "${DD}_uptimerobot" /etc/crontab)" = "" ]; then
                            ...
                            echo "*/2 * * * * root /usr/bin/Cinematika uptimerobot \"${DD}\" \"${3}\" \"${4}\" >>\"/home/${DD}/log/uptimerobot_\$(date '+\%d_\%m_\%Y').log\" 2>&1" >>/etc/crontab
                            echo "# ----- ${DD}_uptimerobot --------------------------------------" >>/etc/crontab
        ...
        "aria2"|"cloudtorrent"|"torrent"|"torrentclient" )
            ...
            if [ "${LOCAL_ACTION}" = "run" ] || [ "${LOCAL_ACTION}" = "1" ]; then
                ...
                while [ "${TORRENT_RUN}" != "50" ]; do
                    ...
                    if [ "`docker ps -aq -f status=running -f name=^/torrent\$ 2>/dev/null`" != "" ]; then
                        docker exec "${CP_DOMAIN_}" /usr/bin/Cinematika container torrent
                        TORRENT_RUN=50
                    fi
                done
                ...
                read_ftp_username "${6}"
                read_ftp_password "${7}"
                FTP_PASSWORD="$(docker exec -t "${CP_DOMAIN_}" /usr/bin/Cinematika rclone_obscure "${FTP_PASSWORD}")"
                            ...
                            if [ -f "/home/${CP_DOMAIN}/files/torrent/.3hours" ]; then
                                if [ -f "/home/${CP_DOMAIN}/files/torrent/.4hours" ]; then
                                    if [ -f "/home/${CP_DOMAIN}/files/torrent/.5hours" ]; then
                                        /usr/bin/Cinematika kill_rclone "${CP_DOMAIN}"
                                        rm -rf \
        ...
        "kill_rclone" )
            ...
            if [ "${CP_OS}" = "alpine" ] || [ "${CP_OS}" = "\"alpine\"" ]; then
                ...
                docker exec -t "${CP_DOMAIN_}" /usr/bin/Cinematika kill_rclone "${CP_DOMAIN}"
            fi
            exit 0
        ;;
        "local" )
            echo ""; echo -n "☐ Change to a local build Cinematika.sh ...";
            sed -Ei "s/# docker build/docker build/g" /usr/bin/Cinematika >/dev/null
            sed -Ei "s/docker pull/# docker pull/g" /usr/bin/Cinematika >/dev/null
            sleep 1
            echo -e "\\r${G}✓ Change to a local build Cinematika.sh ...${NC}"
            echo ""
            exit 0
        ;;
        "rclone_obscure" )
        ...
        "help"|"H"|"--help"|"-h"|"-H" )
            ...
            printf " nohup Cinematika [FULL COMMAND] > log 2>&1; tail -f log"; _br; _br;
            exit 0
        ;;
        "version"|"ver"|"v"|"V"|"--version"|"--ver"|"-v"|"-V" )
            echo "Cinematika ${CP_VER}"
            _br
            echo "Copyright (c) 2014 - ... Cinematika"
            _br
            exit 0
        ;;
        * )
